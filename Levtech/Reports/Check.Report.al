report 50007 CheckPrint
{
    DefaultLayout = RDLC;
    RDLCLayout = 'Levtech\Reports\Check.rdl';
    Caption = 'CheckPrint';
    ApplicationArea = All;
    ...
    dataset
    {
        ...
        dataitem(GenJnlLine; "Gen. Journal Line")
        {
            ...
            column(VendorName; VendorAddr[1]) { }
            column(VendorAddress; VendorAddr[2]) { }
            column(VendorAddress1; VendorAddr[3]) { }
            column(VendorCity; VendorAddr[4]) { }
            column(VendorPostcode; VendorAddr[5]) { }
            column(VendorCountry; VendorAddr[6]) { }
            column(Document_Date; "Document Date") { }
            column(Document_Type; "Document Type") { }
            column(Document_No_; "Document No.") { }
            column(InvDocumentNoG; InvDocumentNoG) { }
            column(InvDocumentTypeG; InvDocumentTypeG) { }
            column(InvRemaningAmountG; InvRemaningAmountG) { }
            column(InvOriginalAmountG; InvOriginalAmountG) { }
            column(InvDateG; InvDateG) { }
                        ...
                        if not TestPrint then begin
                            with GenJnlLine do begin
                                ...
                                if FoundLast and AddedRemainingAmount then begin
                                    ...
                                    //FormatNoText(DescriptionLine, GenJnlLine.Amount, BankAcc2."Currency Code");
                                    VoidText := '';
                                end else begin
            ...
            trigger OnAfterGetRecord()
            var
                PurchaseInvHeaderL: Record "Purch. Inv. Header";
            begin
                ...
                Clear(VendorAddr);
                Clear(VendorRecG);
                if VendorRecG.Get(GenJnlLine."Account No.") then begin
                    VendorAddr[1] := VendorRecG.Name;
                    VendorAddr[2] := VendorRecG.Address;
                    VendorAddr[3] := VendorRecG."Address 2";
                    VendorAddr[4] := VendorRecG.City;
                    VendorAddr[5] := VendorRecG."Post Code";
                    if ZipCodeG.Get(VendorRecG."Post Code") then
                        VendorAddr[6] := VendorRecG."Country/Region Code";
                end;

                Clear(VendorLedgerEntryG);
                Clear(InvDocumentNoG);
                Clear(InvDocumentTypeG);
                Clear(InvOriginalAmountG);
                Clear(InvRemaningAmountG);
                Clear(InvDateG);
                VendorLedgerEntryG.SetRange("Document No.", GenJnlLine."Applies-to Doc. No.");
                if VendorLedgerEntryG.FindFirst() then begin
                    VendorLedgerEntryG.CalcFields("Original Amount", "Remaining Amount");
                    InvDocumentTypeG := Format(VendorLedgerEntryG."Document Type");
                    if PurchaseInvHeaderL.Get(VendorLedgerEntryG."Document No.") then
                        InvDocumentNoG := PurchaseInvHeaderL."Vendor Invoice No.";
                    InvDateG := VendorLedgerEntryG."Posting Date";
                    InvOriginalAmountG := Abs(VendorLedgerEntryG."Original Amount");
                    InvRemaningAmountG := Abs(VendorLedgerEntryG."Remaining Amount");
                end;
    ...
    var
        ...
        ZipCodeG: Record "Post Code";
        StateG: Text;
        InvDocumentTypeG: Text;
        InvDocumentNoG: Text;
        InvRemaningAmountG: Decimal;
        InvOriginalAmountG: Decimal;
        InvDateG: Date;
