report 72150 "Sales - PilotSalesInvoice DCR"
{
    ...
    dataset
    {
        dataitem(SalesHdr; "Sales Header")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                column(SalesPilotInvoiceTitle; STRSUBSTNO(Trl('SalesPilotInvoice%1'), wgCopyText))
                {
                }
                ...
                dataitem(SalesLine; "Sales Line")
                {
                    ...
                    dataitem(ShipmentBuf; "Sales Shipment Line")
                    {
                ...
                dataitem(PrepmtLine; "Prepayment Inv. Line Buffer")
                {
                    ...
                    dataitem(PrepmtDim; "Dimension Set Entry")
                    {
                        DataItemLink = "Dimension Set ID" = FIELD("Dimension Set ID");
                        DataItemTableView = SORTING("Dimension Set ID", "Dimension Code");
                        column(DimText_3; "Dimension Value Name")// wgDimText)//Krishna
                        {
                        }
            ...
            trigger OnAfterGetRecord();
            var
                wlRecTempPrepmtSalesLine: Record "Sales Line" temporary;
                wlRecTempSalesLine: Record "Sales Line" temporary;
            ...
            begin
                ...
                wlCduSalesPost.GetSalesLines(SalesHdr, SalesLine, 1);  //1 = Invoicing
                //Remove lines not to invoice
                ...
                SalesLine.CalcVATAmountLines(1, SalesHdr, SalesLine, VATAmtLine); //1 = Invoicing
                SalesLine.UpdateVATOnLines(1, SalesHdr, SalesLine, VATAmtLine);   //1 = Invoicing
                ...
                wgCduSalesPostPrepmt.GetSalesLines(SalesHdr, 0, wlRecTempPrepmtSalesLine);    //0 = Invoice

                if not wlRecTempPrepmtSalesLine.ISEMPTY then begin
                    wgCduSalesPostPrepmt.GetSalesLinesToDeduct(SalesHdr, wlRecTempSalesLine);
                    if not wlRecTempSalesLine.ISEMPTY then
                        wgCduSalesPostPrepmt.CalcVATAmountLines(SalesHdr, wlRecTempSalesLine, wlRecTempPrepmtVATAmountLineDeduct, 1);
                end;

                //Get Prepayment VAT Amount Lines
                wgCduSalesPostPrepmt.CalcVATAmountLines(SalesHdr, wlRecTempPrepmtSalesLine, PrepmtVATAmtLine, 0);
                ...
                wgCduSalesPostPrepmt.UpdateVATOnLines(SalesHdr, wlRecTempPrepmtSalesLine, PrepmtVATAmtLine, 0);
                //                wgCduSalesPostPrepmt.BuildInvLineBuffer2(SalesHdr, wlRecTempPrepmtSalesLine, 0, PrepmtLine);//krishna
                wgCduSalesPostPrepmt.BuildInvLineBuffer(SalesHdr, wlRecTempPrepmtSalesLine, 0, PrepmtLine);//Krishna

                wgTotPrepmtLineAmount := VATAmtLine.GetTotalLineAmount(SalesHdr."Prices Including VAT", SalesHdr."Currency Code");
    ...
    trigger OnPreReport();
    begin
        //Moved from Sales Header OnPreDataItem
        //Otherwise Print/Excel is not shown
    ...
    var
        ...
        wgCduSalesPostPrepmt: Codeunit "Sales-Post Prepayments";
